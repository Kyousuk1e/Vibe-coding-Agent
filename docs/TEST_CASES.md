...
真实 API 验收脚本有七个自然语言交互阶段：纯对话记忆写入、纯对话追问、计算、搜索、天气与待办、另一窗口周报与待办、工具追问；随后直接读取持久化状态验证恢复和隔离。断言既检查回复，也检查实际工具调用 trace 和待办状态。模型改写指定待办文本或不选择要求的工具会使验收失败，方便发现 Prompt/模型差异，不会宽松地把任意答案当成功。
